...
# as lognormal isn't included for temporal we need to use dempiric. For this we enter the info
# regarding the params of the lognormal. We will estimate these using the same method as for the
# sim that we are comparing too 
...
qtl_sp <- get_quantiles(f_spatial, q, pi = pi)
qtl_sp
...
si_table_vimes_ln <- as.data.frame(matrix(nrow = length(pi_range), ncol = length(q)))
colnames(si_table_vimes_ln) <- q
rownames(si_table_vimes_ln) <- pi_range
#Just getting the quantiles

# increase the precision for these estimates
# not sure how much impact altering precision will have on the lognormal - will need to check the vimes
# function that is used in dempiric
tictoc::tic()
for(i in 1:length(pi_range)){
  si_table_vimes_ln[i,] <- get_quantiles(f_temporal, q, pi = pi_range[i],
