...
si_table_sim_ln <- as.data.frame(matrix(nrow = length(pi_range), ncol = length(q)))
colnames(si_table_sim_ln) <- q
rownames(si_table_sim_ln) <- pi_range
#Just getting the quantiles

for(i in 1:length(pi_range)){
  si_table_sim_ln[i,] <- get_quantiles_sim(d_type = "temporal",
                                              distrib = "lognormal",  n = n, rrpi = pi_range[i], 
                                              params = c(si_mean, si_sd), 
                                              q = q)$threshold_sim
}

#write.csv(si_table_sim_ln, "tests_sh/sim_si_ln_1mill.csv")

#repeat the process to see if we get the same results whilst keeping n the same

si_table_sim_ln_2 <- as.data.frame(matrix(nrow = length(pi_range), ncol = length(q)))
colnames(si_table_sim_ln_2) <- q
rownames(si_table_sim_ln_2) <- pi_range
#Just getting the quantiles

for(i in 1:length(pi_range)){
  si_table_sim_ln_2[i,] <- get_quantiles_sim(d_type = "temporal",
                                                distrib = "lognormal",  n = n, rrpi = pi_range[i], 
                                                params = c(si_mean, si_sd), 
                                                q = q)$threshold_sim
}
...
comp_1mil <- si_table_sim_ln/si_table_sim_ln_2
comp_1mil <- comp_1mil*100 - 100
comp_1mil <- round(comp_1mil, 2)
range(comp_1mil)

# Less than 1% difference between most of the the values, but there are differences. 

# Repeat the process with 5 million creatures

n = 5000000
si_table_sim_ln_5mil <- as.data.frame(matrix(nrow = length(pi_range), ncol = length(q)))
colnames(si_table_sim_ln_5mil) <- q
rownames(si_table_sim_ln_5mil) <- pi_range

for(i in 1:length(pi_range)){
  si_table_sim_ln_5mil[i,] <- get_quantiles_sim(d_type = "temporal",
                                                   distrib = "lognormal",  n = n, rrpi = pi_range[i], 
                                                   params = c(si_mean, si_sd), 
                                                   q = q)$threshold_sim
}

#write.csv(si_table_sim_ln_5mil, "tests_sh/sim_si_ln_5mill.csv")

#repeat the process to see if we get the same results whilst keeping n the same

si_table_sim_ln_5mil_2 <- as.data.frame(matrix(nrow = length(pi_range), ncol = length(q)))
colnames(si_table_sim_ln_5mil_2) <- q
rownames(si_table_sim_ln_5mil_2) <- pi_range

for(i in 1:length(pi_range)){
  si_table_sim_ln_5mil_2[i,] <- get_quantiles_sim(d_type = "temporal",
                                                     distrib = "lognormal",  n = n, rrpi = pi_range[i], 
                                                     params = c(si_mean, si_sd), 
                                                     q = q)$threshold_sim
}
...
comp_5mil <- si_table_sim_ln_5mil/si_table_sim_ln_5mil_2
...
#write.csv(comp_1mil, "tests_sh/compare_1mil_ln.csv")
#write.csv(comp_5mil, "tests_sh/compare_5mil_ln.csv")


# Repeat the process with 10 million creatures

n = 10000000
si_table_sim_ln_10mil <- as.data.frame(matrix(nrow = length(pi_range), ncol = length(q)))
colnames(si_table_sim_ln_10mil) <- q
rownames(si_table_sim_ln_10mil) <- pi_range

tictoc::tic()
for(i in 1:length(pi_range)){
  si_table_sim_ln_10mil[i,] <- get_quantiles_sim(d_type = "temporal",
                                                distrib = "lognormal",  n = n, rrpi = pi_range[i], 
                                                params = c(si_mean, si_sd), 
                                                q = q)$threshold_sim
}
...
#write.csv(si_table_sim_ln_10mil, "tests_sh/sim_si_ln_10mill.csv")

#repeat the process to see if we get the same results whilst keeping n the same

si_table_sim_ln_10mil_2 <- as.data.frame(matrix(nrow = length(pi_range), ncol = length(q)))
colnames(si_table_sim_ln_10mil_2) <- q
rownames(si_table_sim_ln_10mil_2) <- pi_range

tictoc::tic()
for(i in 1:length(pi_range)){
  si_table_sim_ln_10mil_2[i,] <- get_quantiles_sim(d_type = "temporal",
                                                  distrib = "lognormal",  n = n, rrpi = pi_range[i], 
                                                  params = c(si_mean, si_sd), 
                                                  q = q)$threshold_sim
}
...
comp_10mil <- si_table_sim_ln_10mil/si_table_sim_ln_10mil_2
comp_10mil <- comp_10mil*100 - 100
comp_10mil <- round(comp_10mil, 2)

range(comp_10mil)

#write.csv(comp_10mil, "tests_sh/compare_10mil_ln.csv")
