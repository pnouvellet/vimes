...
## First scenario to look at is reporting probability of 0.5 for both groups and
## with the parameters the same
...
# save these results
#write.csv(si_res_df, "tests_sh/final_scenarios/si_95_0.5_1_100.csv")
#write.csv(dist_res_df, "tests_sh/final_scenarios/dist_95_0.5_1_100.csv")


si_res_df <- read.csv("tests_sh/final_scenarios/si_95_0.5_1_100.csv")
dist_res_df <- read.csv("tests_sh/final_scenarios/dist_95_0.5_1_100.csv")
...
#write.csv(trans_res_df, "tests_sh/trans_res_dfs/trans_res_a.csv")
...
colnames(trans_res_df)[apply(trans_res_df, 1, which.min)]

grep("3.5", colnames(trans_res_df))

trans_res_df[,27]


1-pchisq(trans_res_df[12,27], df = 1)


#This shows the column with the value of 3.5 has the lowest chi sqyared value
...
###########################################################
## Will now extract some results from the vimes results using the assortativity value at 1 and at 3.5

res_1 <- vimes_res_list[[1]]
res_1$cutoff

res_3.5 <- vimes_res_list[[26]]
res_3.5$cutoff
...
###########################################
## Repeat for the results with shape_2 = 3.5
...
mean(res_3.5$clusters$size) # number of clusters including singletons

# The cluster membership can be joined with the SE_Tanz data to create a new data set. 

res_3.5_df <- as.data.frame(res_3.5$clusters$membership) # This is the cluster that they are assigned to. 
colnames(res_3.5_df) <- "cluster_memb"

cs_3.5 <- trans_table_fun(res_3.5_df)

csl_3.5 <- cs_3.5 %>%
  group_by(total, trans_type)%>%
  count()

csl_3.5_trios_up <- csl_3.5[which(csl_3.5$total >=3),]

sum(csl_3.5_trios_up$n) # so 24 clusters of >= 3 in total.
sum(csl_3.5_trios_up[which(csl_3.5_trios_up$trans_type %in% c("domestic_only", "wildlife_only")), "n"])
...
gg_res_3.5 <- ggplot(csl_3.5, aes(x = total, y = n, fill = trans_type))  +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = own_cols, name = "Transmission type",
                    #guide = guide_legend(reverse=TRUE),
                    labels = own_labels) +
...
graph_3.5 <- res_3.5$graph
graph_3.5 <- igraph::as_data_frame(graph_3.5)
graph_3.5 <- igraph::graph_from_data_frame(graph_3.5, directed = F)
rn <- as.numeric(get.vertex.attribute(graph_3.5)$name)
graph_3.5 <-  igraph::set.vertex.attribute(graph_3.5, 'species', value = species_vect_graph[rn]) # assign the species to the graph 
