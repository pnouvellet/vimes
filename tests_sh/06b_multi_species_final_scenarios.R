...
## First scenario to look at is reporting probability of 0.5 for group 1 and 0.25 for group 2
...
# save these results
#write.csv(si_res_df, "tests_sh/final_scenarios/si_95_0.5_0.25_1_10.csv")
...
grep("3.1", colnames(trans_res_df))
...
###########################################################
## Will now extract some results from the vimes results using the assortativity value at 1 and at 3.1

res_1 <- vimes_res_list[[1]]
res_1$cutoff

res_3.1 <- vimes_res_list[[22]]
res_3.1$cutoff
...
###########################################
## Repeat for the results with shape_2 = 3.1

table(res_3.1$clusters$size)  #tells us the the size of the assigned clusters
res_3.1$clusters$K

range(res_3.1$clusters$size) #the range of cluster sizes
hist(res_3.1$clusters$size, col = "pink", xlab = "Size of cluster", breaks = seq(-1,9,1),
     main = "Histogram of cluster sizes")

mean(res_3.1$clusters$size) # number of clusters including singletons

# The cluster membership can be joined with the SE_Tanz data to create a new data set. 

res_3.1_df <- as.data.frame(res_3.1$clusters$membership) # This is the cluster that they are assigned to. 
colnames(res_3.1_df) <- "cluster_memb"

cs_3.1 <- trans_table_fun(res_3.1_df)

csl_3.1 <- cs_3.1 %>%
  group_by(total, trans_type)%>%
  count()

csl_3.1_trios_up <- csl_3.1[which(csl_3.1$total >=3),]

sum(csl_3.1_trios_up$n) # so 24 clusters of >= 3 in total.
sum(csl_3.1_trios_up[which(csl_3.1_trios_up$trans_type %in% c("domestic_only", "wildlife_only")), "n"])
...
gg_res_3.1 <- ggplot(csl_3.1, aes(x = total, y = n, fill = trans_type))  +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = own_cols, name = "Transmission type",
                    #guide = guide_legend(reverse=TRUE),
                    labels = own_labels) +
...
graph_3.1 <- res_3.1$graph
graph_3.1 <- igraph::as_data_frame(graph_3.1)
graph_3.1 <- igraph::graph_from_data_frame(graph_3.1, directed = F)
rn <- as.numeric(get.vertex.attribute(graph_3.1)$name)
graph_3.1 <-  igraph::set.vertex.attribute(graph_3.1, 'species', value = species_vect_graph[rn]) # assign the species to the graph 
...
clusters_vect <-cs_3.1$cluster_memb
class(clusters_vect)

cluster_df <- cbind(SE_Tanz, res_3.1_df)
colnames(cluster_df)
...
levels(spdf$Species)
spdf$Species <- droplevels(spdf$Species)
...
#
pts = c(17,19, 17) # setting these to use for species
...
clusters_vect_fivess <- unlist(cs_3.1[which(cs_1$total >4),"cluster_memb"])
...
dev.off()
pdf("tests_sh/plots/maps_scen2.pdf", height = 5, width = 5)
par(mar = c(1,1,1,1))
plot(study_dis, col = "beige")
plot(singles_spdf, add = T, col = cols[singles_spdf$Species], jitter = T, pch = 15, cex = 0.5)#,
title(main = "A", adj = 0.1)
dev.off()

#############################################
## If include the map plots would be good to have the locations of all the cases for comparison.

xy_all <- vd[,c("Longitude", "Latitude")]

spdf_all <- SpatialPointsDataFrame(coords = xy_all, data = vd,
                               proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
levels(spdf_all$Species)
spdf_all$Species <- droplevels(spdf_all$Species)

cols = c("red", "blue", "red") # use this if colouring by species
# use below if colouring by cluster

dev.off()
pdf("tests_sh/plots/maps_all_cases.pdf", height = 5, width = 5)
par(mar = c(1,1,1,1))
plot(study_dis, col = "beige")
plot(spdf_all, add = T, col = cols[spdf_all$Species], jitter = T, pch = 15, cex = 0.5)#,
title(main = "C", adj = 0.1)
dev.off()
